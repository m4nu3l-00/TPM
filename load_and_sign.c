...
void sign(ESYS_CONTEXT *ectx, TPMT_SIGNATURE **signature, ESYS_TR objectHandle){
	TPMT_SIGNATURE *sign;
	TPMT_SIG_SCHEME inScheme = { .scheme = TPM2_ALG_RSAPSS,
								.details = {
									.rsapss = { .hashAlg = TPM2_ALG_SHA256 }
											}
								};
...
void verify(ESYS_CONTEXT *ectx, TPMT_SIGNATURE *signature, ESYS_TR objectHandle){
	char messageFilename[] = "test";
	char *number;
    size_t digestSize, signatureSize;
	int r = open_read_and_close (messageFilename, (void**)&number, &digestSize);
...
tpm2_openssl_load_rc tpm2_openssl_load_private(const char *path,
    ...
    *pub = *template;
    ...
    if (object_auth) {
		...
		printf("priv->sensitiveArea.authValue.size: %u\n", priv->sensitiveArea.authValue.size); 
...
*/
	...
	*/
	...
	char auth[] = "";
	//int i = ESYS_TR_PASSWORD;
	//sprintf(auth, "%d", i);
	char path[] = "key.pem";
	//printf("%d\n", ESYS_TR_PASSWORD);
	openssl_import(ectx, parent, &rsa_key, handle, auth, path);
    ...
    TPMI_DH_PERSISTENT  permanentHandle = TPM2_PERSISTENT_FIRST; 
	rv = Esys_TR_FromTPMPublic(ectx, permanentHandle,
                              ESYS_TR_NONE, ESYS_TR_NONE, ESYS_TR_NONE,
                              &keyHandle);
							  
	TPMT_SIGNATURE *signature= NULL;
	sign(ectx, &signature, keyHandle);
	keyHandle = ESYS_TR_NONE;
    permanentHandle = TPM2_PERSISTENT_FIRST;
	rv = Esys_TR_FromTPMPublic(ectx, permanentHandle,
                              ESYS_TR_NONE, ESYS_TR_NONE, ESYS_TR_NONE,
                              &keyHandle);
	files_save_signature(signature, "test2");
	TPMT_SIGNATURE sign = load_file("test2");
	verify(ectx, &sign, keyHandle);
    if (rv != TSS2_RC_SUCCESS) {
