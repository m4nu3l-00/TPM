...
void write_enc(const char * path, unsigned char * enc_out)
{
        FILE * fp_src;
                              int i;
...
int createNV(ESYS_CONTEXT *ectx, ESYS_TR* nv_index){
...
int writeToNV(ESYS_CONTEXT *ectx, ESYS_TR* nv_index, TPM2B_MAX_NV_BUFFER *write_data){
...
int readFromNV(ESYS_CONTEXT *ectx, ESYS_TR* nv_index, TPM2B_MAX_NV_BUFFER **read_data){
               TPM2B_MAX_NV_BUFFER *data = NULL;
               int rc = Esys_TR_FromTPMPublic(
                              ectx,
                              TPM2_HR_NV_INDEX + 1,
                              ESYS_TR_NONE,
                              ESYS_TR_NONE,
...
int loadKeys(ESYS_CONTEXT *ectx, TPM2B_PRIVATE *outPrivate, TPM2B_PUBLIC *outPublic, ESYS_TR objectHandle, ESYS_TR *key){
...
int signData(ESYS_CONTEXT *ectx, TPMT_SIGNATURE **signature, ESYS_TR objectHandle, char* messageFilename){
               TPMT_SIGNATURE *sign;
               TPMT_SIG_SCHEME inScheme = { .scheme = TPM2_ALG_NULL };
               char *number;
    size_t digestSize, signatureSize;
               // if encrypted better use files_load_bytes_from_path
               int r = open_read_and_close ("path", (void**)&number, &digestSize);
...
int checkSingature(ESYS_CONTEXT *ectx, TPMT_SIGNATURE *signature, char * messageFilename, ESYS_TR keyHandle){
...
int encryptData(char * reading_path, unsigned char* key, char * writing_path){
               AES_KEY enc_key;
               FILE * fp;
               unsigned char * enc;
               unsigned char * text;
    ...
    printf("Test\n");
               text = (char *)malloc(filelen * sizeof(char)); // Enough memory for the file
               fread(text, filelen, 1, fp); // Read in the entire file
    printf("Test\n");
               fclose(fp); // Close the file
               
               printf("%s\n",text);
               enc=(unsigned char *)malloc(filelen * sizeof(char));
    AES_set_encrypt_key(key, 128, &enc_key);
    AES_encrypt(text, enc, &enc_key);
               write_enc(writing_path, enc);
    return 0;
} 

int decryptData(char * reading_path, unsigned char* key){
               AES_KEY dec_key;
               FILE * fp;
               unsigned char * enc;
               unsigned char * dec;
...
int create_primary(ESYS_CONTEXT *ectx, ESYS_TR *objectHandle, char* handle) {
    ...
    bool result = tpm2_util_string_to_uint32(handle, &persist_handle);
    if (!result) {
        ...
        return 1;
               ...
               rv = Esys_EvictControl(ectx, ESYS_TR_RH_OWNER, *objectHandle,
                          ESYS_TR_PASSWORD, ESYS_TR_NONE, ESYS_TR_NONE,
                          persist_handle, &persistent_handle);
              if (rv != TSS2_RC_SUCCESS) {
...
bool tpm2_alg_util_handle_ext_alg(const char *alg_spec, TPM2B_PUBLIC *public) {
    ...
    if (rc < 0 || (size_t) rc >= sizeof(buf)) {
        fprintf(stderr, "tpm2_alg_util_handle_ext_alg\n");
    }
...
tool_rc tpm2_alg_util_public_init(char *alg_details, char *name_halg, char *attrs,
    ...
    if (attrs && tmp.publicArea.objectAttributes !=
        ...
        // printf("Specified attributes \"%s\" and algorithm specifier \"%s\" do "
...
static bool handle_ossl_pass(const char *passin, char **pass) {
    ...
    //     pfn = do_stdin;
    // } else {
    //     printf("Unknown OSSL style password argument, got: \"%s\"", passin);
    //     return false;
    // }
...
static bool load_public_RSA_from_key(EVP_PKEY *key, TPM2B_PUBLIC *pub) {
    ...
    if (!rc) {
        printf("Could not read public modulus ");
        #if OPENSSL_VERSION_NUMBER < 0x30000000L
                                            /* k,n,e point to internal structrues and must not be freed after use */
                              #else
                                            BN_free(n);
    ...
    if (!rc) {
        printf("Could not read public exponent E");
        #if OPENSSL_VERSION_NUMBER < 0x30000000L
                                            /* k,n,e point to internal structrues and must not be freed after use */
                              #else
                                            BN_free(n);
...
tpm2_openssl_load_rc tpm2_openssl_load_private(const char *path,
    ...
    *pub = *template;
    ...
    //     if (passin) {
    //         printf("No password can be used for protecting AES key");
    //         rc = lprc_error;
    //     } else if (template->publicArea.parameters.asymDetail.symmetric.algorithm != TPM2_ALG_AES) {
    //         printf("Cannot handle non-aes symmetric objects, got: 0x%x",
    ...
    // case TPM2_ALG_KEYEDHASH:
    //     if (passin) {
    //         printf("No password can be used for protecting %s key",
...
EVP_PKEY *convert_pubkey_RSA(TPMT_PUBLIC *public) {
    ...
    if (!build) {
        printf("Failed to allocate OpenSSL parameters");
        #if OPENSSL_VERSION_NUMBER < 0x30000000L
                                            RSA_free(rsa_key);
                              #else
                                            EVP_PKEY_CTX_free(ctx);
    ...
    if (!rc) {
        printf("Failed to set RSA modulus");
        #if OPENSSL_VERSION_NUMBER < 0x30000000L
                                            RSA_free(rsa_key);
                              #else
                                            EVP_PKEY_CTX_free(ctx);
    ...
    if (!params) {
        printf("Failed to build OpenSSL parameters");
        #if OPENSSL_VERSION_NUMBER < 0x30000000L
                                            RSA_free(rsa_key);
                              #else
                                            EVP_PKEY_CTX_free(ctx);
    ...
    if (!ctx) {
        printf("Failed to allocate RSA key context");
        #if OPENSSL_VERSION_NUMBER < 0x30000000L
                                            RSA_free(rsa_key);
                              #else
                                            EVP_PKEY_CTX_free(ctx);
    ...
    if (rc <= 0) {
        printf("Failed to initialize RSA key creation");
        #if OPENSSL_VERSION_NUMBER < 0x30000000L
                                            RSA_free(rsa_key);
                              #else
                                            EVP_PKEY_CTX_free(ctx);
    ...
    if (rc <= 0) {
        printf("Failed to create a RSA public key");
        #if OPENSSL_VERSION_NUMBER < 0x30000000L
                                            RSA_free(rsa_key);
                              #else
                                            EVP_PKEY_CTX_free(ctx);
...
void openssl_import(ESYS_CONTEXT *ectx, ESYS_TR new_parent, ESYS_TR *rsa_key, char *key_handle, char *key_auth, char *input_key) {
    ...
    if (tmp_rc != tool_rc_success) {
        ...
        //goto keyout;
               ...
               TPMI_DH_PERSISTENT persist_handle;   
    result = tpm2_util_string_to_uint32(handle, &persist_handle);
    if (!result) {
        fprintf(stderr, "Could not convert persistent handle to a number\n");
        exit(1);
    } else {
               ...
               persist_handle= TPM2_PERSISTENT_FIRST;
               TSS2_RC rv = Esys_EvictControl(ectx, ESYS_TR_RH_OWNER, *rsa_key,
                          ESYS_TR_PASSWORD, ESYS_TR_NONE, ESYS_TR_NONE,
                          persist_handle, &persistent_handle);
